...
class RuwacScraper {
  ...
  async scrapeDetailPage(detailUrl, productFolder) {
    ...
    if (productHeader.length > 0) {
      ...
      for (const line of lines) {
        ...
        if (trimmed.length > 20 && !trimmed.includes(detailInfo.pageTitle) &&
    ...
    const navigationItems = ['blog', 'brochure', 'film', 'clip', 'about', 'news', 'contact',
                           'cookie', 'necessary', 'marketing', 'statistics', 'security', 'how to',
                           'machine registration', 'application examples', 'fairs', 'events', 'did you know'];
    ...
    $('.dropdown-menu').each((i, elem) => {
      ...
      if (headerText.includes('application') || headerText.includes('example')) {
        $elem.find('li').slice(0, 15).each((idx, li) => {
          const liText = $(li).text().trim();
          const lowerText = liText.toLowerCase();

          // Filter out unwanted content
          if (liText && liText.length > 3 && liText.length < 150 &&
              !lowerText.includes('globe') && !lowerText.includes('mailto') &&
              !lowerText.match(/^[a-z]{2}\s*\(/) && !lowerText.match(/^\s*\n/) &&
    ...
    if (applicationsSet.size === 0) {
      $('ul, ol').each((i, list) => {
        ...
        if (parentClass.includes('application') || parentClass.includes('example') ||
            parentText.includes('application') || parentText.includes('example')) {
          $list.find('li').slice(0, 10).each((idx, li) => {
            const liText = $(li).text().trim();
            const lowerText = liText.toLowerCase();
            if (liText && liText.length > 3 && liText.length < 150 &&
                !navigationItems.some(item => lowerText.includes(item))) {
    ...
    for (const img of $('img').toArray()) {
      ...
      if (imgSrc && !['logo', 'icon', 'menu', 'avatar', 'globe', 'flag'].some(x =>
          imgSrc.toLowerCase().includes(x) || alt.includes(x))) {
  ...
  async extractProductsFromCategory($, category, subcategory, categoryUrl) {
    ...
    let productContainers = $('div, article').get().filter(el => {
      const $el = $(el);
    ...
    if (productContainers.length === 0) {
      productContainers = $('div').get().filter(el => {
        const $el = $(el);
        return $el.find('img').length > 0 &&
               ($el.find('h2').length > 0 || $el.find('h3').length > 0 || $el.find('h4').length > 0);
      });
    }
