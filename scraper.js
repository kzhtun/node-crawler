...
class RuwacScraper {
  ...
  async scrapeDetailPage(detailUrl, productFolder) {
    ...
    // Extract images from detail page (excluding decorative ones), store full URLs
    for (const img of $("img").toArray()) {
      ...
      if (imgSrc && !["logo", "icon", "menu", "avatar", "globe", "flag"].some((x) => imgSrc.toLowerCase().includes(x) || alt.includes(x))) {
        // Make absolute URL
        const absoluteUrl = new URL(imgSrc, this.baseUrl).toString();
        detailInfo.detailImages.push(absoluteUrl);
      }
    }
  ...
  async extractProductsFromCategory($, category, subcategory, categoryUrl) {
    ...
    const productMaxCount = productContainers.length;
    ...
    for (let i = 0; i < productMaxCount; i++) {
      try {
        ...
        // await this.generateProductDocx(product, productFolder); // DOCX generation temporarily disabled
